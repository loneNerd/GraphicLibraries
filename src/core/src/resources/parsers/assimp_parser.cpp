...
bool Parsers::AssimpParser::LoadModel(const FS::path& filePath, std::shared_ptr<OGLResources::Model>& model, EAssimpParserFlags assimpFlags)
{
    m_filePath = filePath;
...
void Parsers::AssimpParser::processNode(aiMatrix4x4& transform, aiNode* node, const aiScene* scene, std::list<std::shared_ptr<OGLResources::Mesh>>& meshes)
{
    ...
    for (uint32_t i = 0; i < node->mNumMeshes; ++i)
    {
        ...
        meshes.emplace_back(std::make_shared<OGLResources::Mesh>(vertices, indices, matName.value()));
    }
